...
@onready var flashBtn = $"../UI/flash"
@onready var light = $Neck/Head/Eyes/Camera/SpotLight3D
@onready var JUMP_BTN = $"../UI/JumpBtn"
@onready var RUN_BTN = $"../UI/RunBtn"
@onready var crouchBtn = $"../UI/crouchBtn"
@onready var freelookBtn = $"../UI/free_look"
@onready var PICK_RAY = $Neck/Head/Eyes/Camera/pick_up_ray
var PICK_OBJECT = null
@onready var PICK_TARGET = $Neck/Head/Eyes/Camera/SpringArm3D/pick_target
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("crouch") or crouchBtn.is_pressed() or $RayCast.is_colliding():
		if is_on_floor():
		...
		$SlidingTimer.stop()
		if Input.is_action_pressed("sprint") or RUN_BTN.is_pressed():
	...
	if Input.is_action_pressed("free_look") or freelookBtn.is_pressed() or !$SlidingTimer.is_stopped():
	...
	if Input.is_action_pressed("jump") or JUMP_BTN.is_pressed() and is_on_floor():
